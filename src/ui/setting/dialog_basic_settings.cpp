...
DialogBasicSettings::DialogBasicSettings(MainWindow *parent)
    ...
    //
    // Premium themes first (new design system)
    ui->theme->addItem("Light", "1");
    ui->theme->addItem("Dark", "2");
    ui->theme->addItem("---", "");  // Separator
    ui->theme->addItem("QDarkStyle (Legacy)", "3");
    // Add Qt style factory themes
    for (const auto& style : QStyleFactory::keys()) {
        ui->theme->addItem(style + " (Qt)", style);
    }
    //
    bool ok;
    QString savedTheme = settings.value("theme", "2").toString(); // Default to Dark
    // Find and select saved theme
    int themeIndex = ui->theme->findData(savedTheme);
    if (themeIndex >= 0) {
        ui->theme->setCurrentIndex(themeIndex);
    } else {
        // Fallback: try to match by text for legacy configs
        themeIndex = ui->theme->findText(savedTheme);
        if (themeIndex >= 0) ui->theme->setCurrentIndex(themeIndex);
    }
    //
    connect(ui->theme, static_cast<void (QComboBox::*)(int)>(&QComboBox::currentIndexChanged), this, [=,this](int index) {
        QSettings settings = getSettings();
        QString themeData = ui->theme->currentData().toString();
        QString ui_theme_text = ui->theme->currentText();

        // Skip separator
        if (themeData.isEmpty() && ui_theme_text == "---") {
            return;
        }

        // Apply theme using data value (0,1,2,3 or style name)
        QString themeToApply = themeData.isEmpty() ? ui_theme_text : themeData;
        themeManager->ApplyTheme(themeToApply);
        settings.setValue("theme", themeToApply);
        settings.sync();
    });
