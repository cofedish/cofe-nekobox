...
namespace {
QString githubLatestApi() {
    return AppInfo::RepoApiUrl() + QStringLiteral("/releases/latest");
}

QString githubReleasesApi() {
    return AppInfo::RepoApiUrl() + QStringLiteral("/releases");
}

QString releaseDownloadPrefix() {
    return AppInfo::RepoUrl() + QStringLiteral("/releases/download/");
}

QString releasePathPrefix() {
    return QStringLiteral("/") + AppInfo::RepoSlug() + QStringLiteral("/releases");
}
...
void UpdateService::checkForUpdates(bool manual) {
    ...
    const QUrl apiUrl(includePrerelease ? githubReleasesApi() : githubLatestApi());
...
UpdateService::InstallMode UpdateService::detectInstallMode(QString *reason) const {
    ...
    if (hasDpkgPackageInstalled("cofebox")) {
...
bool UpdateService::isTrustedReleaseAssetUrl(const QUrl &url) {
    ...
    return url.toString().startsWith(releaseDownloadPrefix());
}
...
bool UpdateService::isTrustedReleasePageUrl(const QUrl &url) {
    ...
    if (url.scheme() != "https") return false;
    if (url.host().compare("github.com", Qt::CaseInsensitive) != 0) return false;
    return url.path().startsWith(releasePathPrefix());
}
